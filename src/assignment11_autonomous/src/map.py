...
class MapVisualization:
    ...
    def __init__(self):
        ...
        self.lane1_pub = rospy.Publisher("/point/lookahead1", Marker, queue_size=10)
        self.lane2_pub = rospy.Publisher("/point/lookahead2", Marker, queue_size=10)
        self.lanes_pub = [self.lane1_pub, self.lane2_pub]
        
        ## self.clicked_point_subscriber = rospy.Subscriber("/clicked_point", PointStamped, self.on_click, queue_size=1)
        self.clicked_point_subscriber = rospy.Subscriber("/filtered_map", Odometry, self.on_click, queue_size=1)
    ...
    def on_click(self, point_msg):
        ...
        position = point_msg.pose.pose.position
        ...
        for lane in self.map.lanes:
            ...
            ## Outcommentet, no use and/or redundant
            ...
            rospy.loginfo("Lookahead of lane[{i}] at [{position.x},{position.y}]")
